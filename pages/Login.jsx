...
const Login = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      // Map toast
      toast.success(untils.mess("login.toast.google_success"));
      navigate("/"); 
  ...
  const handleSubmit = async(e) => {
    ...
    try {
      ...
      if (user.isAdmin) {
        // Map toast
        toast.success(untils.mess("login.toast.login_success"));
        navigate('/admin/dashboard');
      } 
      else if(!user.is_active) {
        // Map toast
        toast.success(untils.mess("login.toast.verify_redirect"));
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 md:p-10 border border-gray-100">
        <div className="text-center mb-8">
          <Link to="/" className="inline-flex items-center gap-2 mb-4 group">
            <div className="size-10 text-primary">
              <svg className="w-full h-full" fill="none" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg">
                <path clipRule="evenodd" d="M24 4C12.9543 4 4 12.9543 4 24C4 35.0457 12.9543 44 24 44C35.0457 44 44 35.0457 44 24C44 12.9543 35.0457 4 24 4ZM24 10C26.2091 10 28 11.7909 28 14V17H31C32.6569 17 34 18.3431 34 20V34C34 35.6569 32.6569 37 31 37H17C15.3431 37 14 35.6569 14 34V20C14 18.3431 15.3431 17 17 17H20V14C20 11.7909 21.7909 10 24 10ZM24 29C25.6569 29 27 27.6569 27 26C27 24.3431 25.6569 23 24 23C22.3431 23 21 24.3431 21 26C21 27.6569 22.3431 29 24 29Z" fill="currentColor" fillRule="evenodd"></path>
              </svg>
            </div>
          ...
          <h1 className="text-3xl font-bold text-[#181411] mb-2">
            {untils.mess("login.welcome_title")}
          </h1>
          <p className="text-gray-500">
            {untils.mess("login.welcome_desc")}
          </p>
        </div>
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="email">
              {untils.mess("login.labels.email")}
            </label>
            <div className="relative">
              ...
              <input 
                ...
                placeholder={untils.mess("login.placeholders.email")}
                type="text" 
                required
              />
            </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input 
                className="block w-full pl-10 pr-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors text-sm" 
                id="password" 
                placeholder={untils.mess("login.placeholders.password")}
                type="password" 
                required
                value={password}
          ...
          <div className="flex items-center">
            ...
            <label className="ml-2 block text-sm text-gray-500" htmlFor="remember-me">
              {untils.mess("login.labels.remember_me")}
            </label>
          </div>
          <button 
            type="submit"
          ...
          >
            {untils.mess("login.btn.login")}
          </button>
        </form>

        <div className="relative my-6">
          <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-gray-200"></div></div>
          <div className="relative flex justify-center text-sm"><span className="px-2 bg-white text-gray-500">
            {untils.mess("login.divider")}
          </span></div>
        </div>
        ...
        <div className="">
          ...
          >
            ...
            {untils.mess("login.btn.google")}
          </button>
        </div>
        
        <div className="mt-6 text-center text-sm text-gray-500">
          {untils.mess("login.links.signup_prompt")} <Link className="font-bold text-primary hover:underline" to="/signup">
            {untils.mess("login.links.signup_link")}
          </Link>
        </div>
      </div>
    </div>
